function [ waveform, plot_time, data ] = FxEIT_timeplot_manual2( inv_Sense, EIT_data, time, EIT_ref, start, finish, sigma_f )
...
for i=1:fix(size(EIT_data,2)/calculation_size)
    temp = sum(((inv_Sense * (repmat(Temp_ref,1,calculation_size) - EIT_data(:,(i-1)*calculation_size+1:(i)*calculation_size))) .* repmat(sigma_f,1,calculation_size)),1);
    waveform((i-1)*calculation_size+1:i*calculation_size) = temp;
    if nargout ==3
        temp2 = ((inv_Sense * (repmat(Temp_ref,1,calculation_size) - EIT_data(:,(i-1)*calculation_size+1:(i)*calculation_size))) .* repmat(sigma_f,1,calculation_size));
        data(1:size(temp2,1),(i-1)*calculation_size+1:i*calculation_size) = temp2;
    end
end
temp = sum(((inv_Sense * (repmat(Temp_ref,1,size(EIT_data,2)-i*calculation_size) - EIT_data(:,i*calculation_size+1:end))) .* repmat(sigma_f,1,size(EIT_data,2)-i*calculation_size)),1);
waveform(i*calculation_size+1:size(EIT_data,2)) = temp;
if nargout ==3
    temp2 = ((inv_Sense * (repmat(Temp_ref,1,size(EIT_data,2)-i*calculation_size) - EIT_data(:,i*calculation_size+1:end))) .* repmat(sigma_f,1,size(EIT_data,2)-i*calculation_size));
    data(1:size(temp2,1),i*calculation_size+1:size(EIT_data,2)) = temp2;
end
