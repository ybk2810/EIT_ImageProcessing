function [ imgscale ] = FxEIT_timeplot_OSA2( Image, time, plot_img, SNOR, start, finish, imgscale, Cmap3, Tidal_img_contour_outside )
...
figure(1); 
subplot(3,3,[1 2 3]); plot(plot_img);
set(gca,'xlim',([plot_time.start plot_time.finish]));
...
subplot(3,3,[4 5 6]); plot(SNOR)
set(gca,'xlim',([1 length(SNOR)]));
set(gca,'xtick',1:(length(SNOR)-1)/(9-1):length(SNOR));
set(gca,'xticklabel',{plot_time.time(:).time}); xlabel('Time(hh:mm:ss)'); ylabel('SNOR');

% Tidal_img = Image(:,:,th_finish) - Image(:,:,th_start);
% imgscale = max(max(Tidal_img));

subplot(3,3,[7]);imagesc(Image(51:211,13:245,301)); colormap(Cmap3);
set(gca,'YDir','normal');set(gca,'xdir','normal'); colorbar('westoutside');
set(gca,'xtick',[],'ytick',[]); axis image; caxis([-abs(imgscale)/5 abs(imgscale)]);
hold on; plot(Tidal_img_contour_outside.ROI_IMG(:,2),Tidal_img_contour_outside.ROI_IMG(:,1),'w','LineWidth',1);
title('C1');

subplot(3,3,[8]);imagesc(Image(51:211,13:245,527)); colormap(Cmap3);
set(gca,'YDir','normal');set(gca,'xdir','normal');
set(gca,'xtick',[],'ytick',[]);axis image;caxis([-abs(imgscale)/5 abs(imgscale)]);
hold on; plot(Tidal_img_contour_outside.ROI_IMG(:,2),Tidal_img_contour_outside.ROI_IMG(:,1),'w','LineWidth',1);
title('C2');

subplot(3,3,[9]);imagesc(Image(51:211,13:245,683)); colormap(Cmap3);
set(gca,'YDir','normal');set(gca,'xdir','normal');
set(gca,'xtick',[],'ytick',[]);axis image;caxis([-abs(imgscale)/5 abs(imgscale)]);
hold on; plot(Tidal_img_contour_outside.ROI_IMG(:,2),Tidal_img_contour_outside.ROI_IMG(:,1),'w','LineWidth',1);
title('C3');
