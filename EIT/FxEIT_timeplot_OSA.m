function [ imgscale ] = FxEIT_timeplot_OSA( th_start, th_finish, Image, time, plot_img, PPG, SPO2, start, finish, Cmap3, Tidal_img_contour_outside )
...
figure(1); 
subplot(4,3,[1 2 3]); plot(plot_img);
set(gca,'xlim',([plot_time.start plot_time.finish]));
set(gca,'ylim',([min(plot_img)-1000000 max(plot_img)+1000000]));
set(gca,'xtick',plot_time.start:(plot_time.finish-plot_time.start)/(length(plot_time.section)-1):plot_time.finish);
set(gca,'xticklabel',{plot_time.time(:).time}); xlabel('Time(hh:mm:ss)'); ylabel('Lung EIT');


subplot(4,3,[4 5 6]); plot(PPG)
set(gca,'xlim',([1 length(PPG)]));
set(gca,'xtick',1:(length(PPG)-1)/(9-1):length(PPG));
set(gca,'xticklabel',{plot_time.time(:).time}); xlabel('Time(hh:mm:ss)'); ylabel('PPG');

subplot(4,3,[7 8 9]); plot(SPO2)
set(gca,'xlim',([1 length(SPO2)]));
set(gca,'ylim',([min(SPO2)-5 max(SPO2)+5]));
set(gca,'xtick',1:(length(SPO2)-1)/(9-1):length(SPO2));
set(gca,'xticklabel',{plot_time.time(:).time}); xlabel('Time(hh:mm:ss)'); ylabel('SPO2');

Tidal_img = Image(:,:,th_finish) - Image(:,:,th_start);
imgscale = max(max(Tidal_img));

subplot(4,3,[10]);imagesc(Image(55:206,15:242,241)); colormap(Cmap3);
set(gca,'YDir','normal');set(gca,'xdir','normal'); colorbar('westoutside');
set(gca,'xtick',[],'ytick',[]);axis image;caxis([-imgscale/5 imgscale]);
hold on; plot(Tidal_img_contour_outside.ROI_IMG(:,2),Tidal_img_contour_outside.ROI_IMG(:,1),'w','LineWidth',1);
title('C1');

subplot(4,3,[11]);imagesc(Image(55:206,15:242,652)); colormap(Cmap3);
set(gca,'YDir','normal');set(gca,'xdir','normal');
set(gca,'xtick',[],'ytick',[]);axis image;caxis([-imgscale/5 imgscale]);
hold on; plot(Tidal_img_contour_outside.ROI_IMG(:,2),Tidal_img_contour_outside.ROI_IMG(:,1),'w','LineWidth',1);
title('C2');

subplot(4,3,[12]);imagesc(Image(55:206,15:242,1106)); colormap(Cmap3);
set(gca,'YDir','normal');set(gca,'xdir','normal');
set(gca,'xtick',[],'ytick',[]);axis image;caxis([-imgscale/5 imgscale]);
hold on; plot(Tidal_img_contour_outside.ROI_IMG(:,2),Tidal_img_contour_outside.ROI_IMG(:,1),'w','LineWidth',1);
title('C3');
