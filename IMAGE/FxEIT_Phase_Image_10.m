function [ Phase_Image ] = FxEIT_Phase_Image_10( Data, EIT_ref, DataSet, fs1 )
...
for cntPEEP = 1:length(DataSet.PEEP) % PEEP
    for cntBreath = 2:length(DataSet.PEEP(cntPEEP).index)-1 % Breath        
        ...
        Temp = Data.Proj_Mat * DataSet.EIT_filt_L(:,a-fs1:a+fs1*2-1); % one breath % boundary artifact remover
...
%         [~, avg_peak] = min(mean(sigma2(:,w/2:end-w/2)));
%         avg_peak=avg_peak+w/2;
...
%     patch('Faces',Data.Element,'Vertices' ,Data.Node,'FaceVertexCData' ,sigma_peak3(:,8),'FaceColor' ,'flat' ,'EdgeColor' ,'None' );
% axis equal; caxis([-4 4]); axis off;
