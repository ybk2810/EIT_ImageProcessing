...
%     Cmap3 = FxImage_Cmap(3); % import double side colormap
    ...
    for i = 1:size(Image,3)
        ...
        FxEIT_PlotBoundary(ROI,'w',1,2);
        title(['Pressure : ',num2str(PEEP(i))],'Color','w');
    ...
    caxis([-abs(max(Cmax_tab(~isinf(Cmax_tab)))*1.1) abs(max(Cmax_tab(~isinf(Cmax_tab)))*1.1)]);
    FxEIT_PlotBoundary(background_mask,'w',2);
    FxEIT_PlotBoundary(ROI,'w',1,2);
    set(gca,'Ydir','normal'); axis image off; title('C_m_a_x Image');
    subplottight(3,size(Image,3)+1,size(Image,3)+3);
    colormap(Cmap3); caxis([-abs(max(max(Cmax(isfinite(Cmax))))) abs(max(max(Cmax(isfinite(Cmax)))))]);
    h = colorbar('Location','west'); set(h,'ylim',[0,abs(max(max(Cmax(isfinite(Cmax)))))]);
    axis off;
