function [ Phase_Image ] = FxEIT_Phase_Image( Data, EIT_ref, DataSet, type )
...
elseif type==2
    % fEIT(Tidal Volume) image RIO masking Image
    a = DataSet.Breath(DataSet.TidalVolume(1).index(2)).index(1); % start
    b = DataSet.Breath(DataSet.TidalVolume(1).index(end-1)).index(end); % finish
    Temp_f = Data.Proj_Mat * DataSet.EIT_filt_L(:,a:b); % boundary artifact remover
    sigma_f = Data.inv_Sense_weighted_avg*Temp_f; % Sensitivity matrix(weighted method)
    sigma_f = std(sigma_f');
    sigma_f = sigma_f';
    ...
    for cntTidalVolume = 1:length(DataSet.TidalVolume) % PEEP
        for cntBreath = 2:length(DataSet.TidalVolume(cntTidalVolume).index)-1 % Breath        
            a = DataSet.Breath(DataSet.TidalVolume(cntTidalVolume).index(cntBreath)).inhale;
            Temp = Data.Proj_Mat * DataSet.EIT_filt_L(:,a-50:a+99); % one breath % boundary artifact remover
    ...
    %         axis equal; caxis([-80 80]); axis off;
        ...
        F = TriScatteredInterp(xy(:,1),xy(:,2),sigma_peak4);
        Phase_Image(:,:,cntTidalVolume) = F(qx,qy);

        disp(num2str(cntTidalVolume))
    end
end
end
